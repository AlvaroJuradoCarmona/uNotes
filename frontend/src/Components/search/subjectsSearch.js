...
export default function ControllableStates() {
  ...
  async function fetchData() {
    try {
      ...
      const subjectOptions = subjectsData[0].map(subject => ({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', opacity: '100%' }}>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} />}
      />
      <Link className="searchButton" variant="contained" id="doneButton" onClick={() => {
        if (value) {
          handleRowClick(value.id);
        }
      }} style={{ textDecoration: 'none', color: 'white', fontSize: 19 }}>Buscar</Link>
    </div>
  );
}*/