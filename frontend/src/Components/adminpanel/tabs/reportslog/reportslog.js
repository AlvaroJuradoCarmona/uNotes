...
import DeleteReportModal from './modals/deletereport'
...
export default function CustomPaginationActionsTable() {
  ...
  async function fetchData() {
    try {
      const reportsData = await fileService.getReports();
      ...
      <Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={reports.idDocument}>
              <TableCell style={{ width: 40 }} align="right">
                {reports.idCategory >= 6 ? (
              ...
              <TableCell style={{ width: 30 }}>
                <IconButton onClick={() => handleFile(reports.idDocument)}>
                  <ArrowCircleUpRoundedIcon />
                </IconButton>
              </TableCell>
              <TableCell style={{ width: 30 }}>
                <DeleteReportModal idReport={reports.idReport}/>
              </TableCell>
            </TableRow>
          ))}
          {emptyRows > 0 && (
